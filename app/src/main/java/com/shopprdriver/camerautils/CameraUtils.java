package com.shopprdriver.camerautils;
...
public class CameraUtils {
    ...
    public static Uri getOutputMediaFileUri(Context context) {
        File mediaStorageDir = new File(
                context.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "Fynoo");
    ...
    public static Uri getOutputMediaVideo(Context context) {
        File mediaStorageDir = new File(
                context.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "Fynoo");
        //If File is not present create directory
        if (!mediaStorageDir.exists()) {
        ...
        String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss",
                Locale.getDefault()).format(new Date());//Get Current timestamp
        File mediaFile = new File(mediaStorageDir.getPath() + File.separator
                + "Video_" + timeStamp + ".mp4");//create image path with system mill and image format
        return Uri.fromFile(mediaFile);
